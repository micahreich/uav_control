import numpy as np
from spatialmath.base import q2r, qconj, qvmul, r2q, qqmul, skewa

from hybrid_ode_sim.simulation.base import DiscreteTimeModel
from uav_control.constants import decompose_state, compose_state_dot, g, a_g_N, e3_B, thrust_axis_B, e3_N
...
class QuadrotorGeometricControllerParams:
    ...
    I: np.ndarray = field(default_factory=lambda: np.eye(3))   # inertia matrix of the quadrotor
...
class QuadrotorGeometricController(DiscreteTimeModel):
    def __init__(self, y0: np.ndarray, sample_rate: int, params = QuadrotorGeometricControllerParams()):
    ...
    def discrete_dynamics(self, _t: float, _y: np.ndarray) -> np.ndarray:
        ...
        [b_1d, b_1d_dot, b_1d_ddot] = self.input_models['dfb_planner'].y
        ...
        A_dot = -self.params.kP @ e_v + \
                -self.params.kD @ e_a + \
                self.params.m * j_b0_N_ref
        ...
        A_ddot = -self.params.kP @ e_a + \
                 -self.params.kD @ e_j + \
                 self.params.m * s_b0_N_ref
        
        b_3c_ddot = compute_unit_vector_ddot(A, A_dot, A_ddot)
        
        C_ddot = compute_cross_product_dot(b_3c_dot, b_3c_ddot, b_1d, b_1d_dot) + \
                 compute_cross_product_dot(b_3c, b_3c_dot, b_1d_dot, b_1d_ddot)
                 
        b_2c_ddot = compute_unit_vector_ddot(C, C_dot, C_ddot)
        
        b_1c_ddot = compute_cross_product_dot(b_2c_dot, b_2c_ddot, b_3c, b_3c_dot) + \
