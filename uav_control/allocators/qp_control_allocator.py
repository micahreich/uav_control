...
class QuadrotorQPAllocator(DiscreteTimeModel):
    def __init__(self, y0: np.ndarray, sample_rate: int, params = QuadrotorQPAllocatorParams()):
        ...
        for i, r in enumerate([
            self.params.l * np.array([1., 0., 0.]),
            self.params.l * np.array([0., 1., 0.]),
            self.params.l * np.array([-1., 0., 0.]),
            self.params.l * np.array([0., -1., 0.])
        ]):
            self.G1[1:, i] = np.cross(r, e3_B)
        
        self.G1[3, :] += [self.params.cq_ct_ratio, -self.params.cq_ct_ratio, self.params.cq_ct_ratio, -self.params.cq_ct_ratio]
    ...
    def discrete_dynamics(self, _t: float, _y: np.ndarray) -> np.ndarray:
        ...
        constraint_vector = np.hstack((
            self.params.u_max * np.ones(4),
            -self.params.u_min * np.ones(4)
        ))
        
        P = self.G1.T @ self.params.W @ self.G1
        q = -2 * self.G1.T @ self.params.W @ control_wrench
        
        prob = cp.Problem(cp.Minimize(cp.quad_form(u, P) + q.T @ u),
