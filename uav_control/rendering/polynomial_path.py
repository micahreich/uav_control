...
class PolynomialTrajectory(PlotElement):
    def __init__(self, env: PlotEnvironment, polytraj,
        ...
        if color_velocities:
            ...
            self.env.ax.plot(positions[:, 0], positions[:, 1], positions[:, 2], color='gray', linestyle='dashed', lw=1.0)

        # Plot waypoints of the trajectory
        if plot_waypoints:
            self.env.ax.scatter(polytraj.waypoints[:, 0], polytraj.waypoints[:, 1], polytraj.waypoints[:, 2],
                                color='black', alpha=0.6, s=10)
        
if __name__ == "__main__":
    from hybrid_ode_sim.systems.polynomial_trajgen import PolynomialTrajectoryND, visualize_spatial_trajectory
