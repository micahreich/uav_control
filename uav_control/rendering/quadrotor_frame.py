...
class QuadrotorFrame(PlotElement):
    def __init__(self, env: PlotEnvironment, system: Optional[BaseModel],
        ...
        self.x_axis, = self.env.ax.plot(self.rotated_motor_positions[0, :2],
                                       self.rotated_motor_positions[1, :2],
                                       self.rotated_motor_positions[2, :2],
                                       marker='o', markersize=3.0, color='black', linewidth=2.0)
        
        self.y_axis, = self.env.ax.plot(self.rotated_motor_positions[0, 2:],
                                       self.rotated_motor_positions[1, 2:],
                                       self.rotated_motor_positions[2, 2:],
                                       marker='o', markersize=3.0, color='black', linewidth=2.0)
        ...
        for i, motor_position in enumerate(self.rotated_motor_positions.T):
            ...
            rotor_plot, = self.env.ax.plot(i_rotor_coords_B[0, :],
                                 i_rotor_coords_B[1, :],
                                 i_rotor_coords_B[2, :], color='gray', linewidth=1.0)
    ...
    def update(self, t):        
        ...
        for i, _ in enumerate(self.rotated_motor_positions.T):            
            ...
            self.set_plot_xyz(self.rotors_plots[i],
...
if __name__ == "__main__":
    ...
    coordinate_frame = QuadrotorFrame(env, system=None,
