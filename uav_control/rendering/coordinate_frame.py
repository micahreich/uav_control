...
from spatialmath.base import q2r, r2q, rotz, qunit, qslerp, qconj
...
class CoordinateFrame(PlotElement):
    def __init__(self, env: PlotEnvironment, system: Optional[BaseModel],
                 show_origin=True,
        ...
        if show_origin:
            ...
            for v in np.eye(3):
                self.env.ax.quiver(0, 0, 0, *v, color='black', arrow_length_ratio=0.05, lw=1.0, alpha=0.5)
        
        self.frame_x_text = self.env.ax.text(0, 0, 0, "x", color='r', fontsize=12, fontfamily='monospace', va='center', ha='center')
        self.frame_y_text = self.env.ax.text(0, 0, 0, "y", color='g', fontsize=12, fontfamily='monospace', va='center', ha='center')
        self.frame_z_text = self.env.ax.text(0, 0, 0, "z", color='b', fontsize=12, fontfamily='monospace', va='center', ha='center')
    ...
    def set_text_location(self, text_locations):
        for i, (text, axis) in enumerate(zip([self.frame_x_text, self.frame_y_text, self.frame_z_text],
                                             ['y', 'x', 'x'])):
    ...
    def update(self, t):        
        ...
        text_locations = (rot_NB @ np.array([[1.2, 0, 0], [0, 1.2, 0], [0, 0, 1.2]]).T) + r_b0_N[:, None]
        self.set_text_location(text_locations)
        
        return self.frame_x_vec, self.frame_y_vec, self.frame_z_vec, \
...
if __name__ == "__main__":
    ...
    ys = np.array([
        compose_state(r_b0_N=R @ np.array([1.,0.,0.]), q_NB=r2q(R)) for R in orientations
    ])
    ...
    def system_history_fn(t):
        ...
        return compose_state(
            r_b0_N=qvmul(qnm, np.array([1,0,0])),
            q_NB=qnm
        )

    env = PlotEnvironment(fig, ax, sim_t_range=[0.0, 4.0], frame_rate=60)
    coordinate_frame = CoordinateFrame(env, system=None,
                                       _debug_history=(ts, ys, system_history_fn))
