...
from uav_control.controllers.geometric_controller import compute_unit_vector_ddot, compute_unit_vector_dot
...
class QuadrotorPolynomialPlanner(DiscreteTimeModel):
    def __init__(self, y0: Any, sample_rate: int, params: QuadrotorPolynomialPlannerParams):
        ...
        self.b1d_prev, self.b1d_dot_prev, self.b1d_ddot_prev = e1_N, np.zeros(3), np.zeros(3)
    ...
    def search_nearest_point(self, state: np.ndarray, t0):
        ...
        for _ in range(100):
            ...
            t_new = np.clip(t_new, self.polynomial_traj.timepoints[0], self.polynomial_traj.timepoints[-1])
    ...
    def discrete_dynamics(self, t: float, _y: Any) -> Any:
        ...
        [r_b0_N_ref, v_b0_N_ref, a_b0_N_ref, j_b0_N_ref, s_b0_N_ref] = self.polynomial_traj(t_nearest, n_derivatives=4)
        ...
        if v_b0_N_ref_planar_norm > 0.01:
            ...
            b1d_ddot = compute_unit_vector_ddot(v_b0_N_ref_planar, a_b0_N_ref_planar, j_b0_N_ref_planar)
            
            self.b1d_prev, self.b1d_dot_prev, self.b1d_ddot_prev = b1d, b1d_dot, b1d_ddot
        else:
            b1d, b1d_dot, b1d_ddot = self.b1d_prev, self.b1d_dot_prev, self.b1d_ddot_prev
