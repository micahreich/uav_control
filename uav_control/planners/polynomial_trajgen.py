...
def _qp_constraints(waypoints, times,
                    order=7,
    ...
    n_constraints = n_constraints_endpoint_matches + \
                    n_constraints_deriv_continuity + \
                    n_constraints_start_from_rest + \
                    n_constraints_end_at_rest
                    
    polyderivs = [
        np.polyder(np.ones(N+1), m=i) for i in range(0, N + 1)
    ]
...
def _qp_objective_quad_form(waypoints, times,
                            order=7,
                            minimize_order=1):
    N = order
    M = len(waypoints) - 1
...
def traj_opt_1d(waypoints, times,
                order=7,
                n_constrained_end_derivs=2,
                minimize_order=None) -> np.ndarray:
    ...
    A, b = _qp_constraints(waypoints=waypoints,
                           times=times,
                           order=order,
    ...
    H = _qp_objective_quad_form(waypoints=waypoints,
                                times=times,
                                order=order,
                                minimize_order=minimize_order)
...
class PolynomialTrajectoryND:
    def __init__(self, waypoints, timepoints,
                 order=7,
                 n_constrained_end_derivs=2,
                 minimize_order=None):
    ...
    def _generate_polynomial_coeffs(self):
        return np.array([
            traj_opt_1d(waypoints=self.waypoints[:, i],
...
def visualize_spatial_trajectory(traj: PolynomialTrajectoryND):
    ...
    if traj.dim == 1:
        ...
        ax.plot(xys[:, 0], xys[:, 1], linestyle='--', linewidth=1, color='gray', label="Path")  # 'o' adds markers for each point
        ax.scatter(traj.waypoints[:, 0], traj.waypoints[:, 1], marker='o', color='red', label="Waypoints")
        
        for i in range(len(traj.waypoints)):
            ax.text(traj.waypoints[i, 0] + 0.1, traj.waypoints[i, 1] + 0.1, f"{i}",
                    fontsize=12, color='red', fontfamily='monospace', va='center', ha='center')
        ...
        ax.plot(xyzs[:, 0], xyzs[:, 1], xyzs[:, 2], linestyle='--', linewidth=1, color='gray', label="Path")
        ax.scatter(traj.waypoints[:, 0], traj.waypoints[:, 1], traj.waypoints[:, 2], color='red', label="Waypoints")
        
        for i in range(len(traj.waypoints)):
            ax.text(traj.waypoints[i, 0] + 0.1, traj.waypoints[i, 1] + 0.1, traj.waypoints[i, 2] + 0.1, f"{i}",
                    fontsize=12, color='red', fontfamily='monospace', va='center', ha='center')
